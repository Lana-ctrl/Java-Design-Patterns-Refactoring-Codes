...
public class Board extends JPanel implements Runnable, Commons {
    ...
    public Board() {
        ...
        // Initialize GameEngine AFTER player is created in gameInit()
        // Don't create it here yet!
    ...
    public void gameInit() {
        ...
        // Initialize facade/command engine AFTER player is created
    ...
    // --- YELLOW DASH GLOW EFFECT when dash is active ---
    if (player.isDashingEffectActive()) {
        int px = player.getX();
        int py = player.getY();
        int w = PLAYER_WIDTH;
        int h = PLAYER_HEIGHT;
        ...
        g2d.setColor(new java.awt.Color(255, 255, 0, 80));
        g2d.fillOval(px - 10, py - 10, w + 20, h + 20);

        // Inner stronger yellow glow
        g2d.setColor(new java.awt.Color(255, 255, 0, 140));
        g2d.fillOval(px - 6, py - 6, w + 12, h + 12);
    }
...
public void animationCycle() {
    ...
    // --- SHOT LOGIC ---
    if (shot.isVisible()) {
        // Update shot position using Strategy pattern
        shot.update();
        ...
        for (Alien alien : aliens) {
            if (alien.isVisible() && shot.isVisible()) {
                ...
                if (shotX >= alienX && shotX <= (alienX + ALIEN_WIDTH) &&
                    ...
                    alien.setDying(true);
                    deaths++;
                    shot.setVisible(false); // Hide the shot
        ...
        // Boundary check - if shot goes off screen, hide it
        // This should be handled by the strategy, but keep as backup
        if (shot.getY() < 0) {
